...
{
    public class ReleaseController : UmbracoAuthorizedApiController
    {
        ...
        {
            var allText = File.ReadAllText(HostingEnvironment.MapPath(YouTrackJsonFile));
        ...
        public string SaveAllToFile()
        {
            try
            {
                ...
                if (typedData.Any())
                {
                    ...
                    using (var streamWriter = new StreamWriter(HttpContext.Current.Server.MapPath(YouTrackJsonFile), false))
                    {
            ...
            return string.Format("Results succesfully written to {0} at {1}", HttpContext.Current.Server.MapPath(YouTrackJsonFile), DateTime.Now);
        }
